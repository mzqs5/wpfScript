            ...
            for (int i = 0; i < model.ljCount; i++)
            {
                if (OpenMall(Resource1.活动))
                {
                    ...
                    while (true)
                    {
                        Confirm();
                        Thread.Sleep(500);
                        var r = Bg.FindPic(handle, Resource1.论剑匹配成功, new XRECT() { Left = 630, Top = 410, Right = 670, Bottom = 450 }, 0.9f);
                        if (!r.IsEmpty)
                            break;
                        Thread.Sleep(1500);
                    }
                    ...
                    for (int j = 0; j < 5; j++)
                    {
                        var r = Bg.FindPic(handle, Resource1.关闭设置, new XRECT() { Left = 1127, Top = 48, Right = 1185, Bottom = 100 }, 0.9f);
                        if (!r.IsEmpty)
                        {
